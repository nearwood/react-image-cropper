...
var Cropper = React.createClass({
    ...
    initStyles: function initStyles() {
        ...
        this.setState({
            ...
            if (selectionNatural) {
                ...
                var _rateWidth = imageWidth / img.naturalWidth;
                var _rateHeight = imageHeight / img.naturalHeight;
                var realWidth = parseInt(frameWidth * _rateWidth);
                var realHeight = parseInt(frameHeight * _rateHeight);
                var realX = parseInt(originX * _rateWidth);
                var realY = parseInt(originY * _rateHeight);
        ...
        setTimeout(function () {
            ...
            if (img && img.naturalWidth) {
                ...
                var _that$props = that.props,
                    imgWidthMax = _that$props.imgWidthMax,
                    imgHeightMax = _that$props.imgHeightMax;
    ...
    frameDotMove: function frameDotMove(dir, e) {
        ...
        if (pageY !== 0 && pageX !== 0) {
            ...
            var rateW = rate / widthRatio;
            var rateH = rate * heightRatio;

            switch (dir) {
                case 'ne':
                    new_height = frameHeight - _y;
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : new_height, originX, fixedRatio ? originY - _x / rateW : originY + _y);
                case 'e':
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : frameHeight, originX, fixedRatio ? originY - _x / rateW * 0.5 : originY);
                case 'se':
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : new_height, originX, originY);
                case 'n':
                    new_height = frameHeight - _y;
                    return this.calcPosition(fixedRatio ? new_height * rateH : frameWidth, new_height, fixedRatio ? originX + _y * rateH * 0.5 : originX, originY + _y);
                case 'nw':
                    new_width = frameWidth - _x;
                    new_height = frameHeight - _y;
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : new_height, originX + _x, fixedRatio ? originY + _x / rateW : originY + _y);
                case 'w':
                    new_width = frameWidth - _x;
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : frameHeight, originX + _x, fixedRatio ? originY + _x / rateW * 0.5 : originY);
                case 'sw':
                    new_width = frameWidth - _x;
                    return this.calcPosition(new_width, fixedRatio ? new_width / rateW : new_height, originX + _x, originY);
                case 's':
                    new_height = frameHeight + _y;
                    return this.calcPosition(fixedRatio ? new_height * rateH : frameWidth, new_height, fixedRatio ? originX - _y * rateH * 0.5 : originX, originY);
                default:
                    return;
            }
        }
    ...
    crop: function crop() {
        var _state8 = this.state,
            ...
            imageWidth = _state8.imageWidth,
            imageHeight = _state8.imageHeight;

        var canvas = document.createElement('canvas');
        var img = ReactDOM.findDOMNode(this.refs.img);
        var _rateWidth = img.naturalWidth / imageWidth;
        var _rateHeight = img.naturalHeight / imageHeight;
        var realWidth = frameWidth * _rateWidth;
        var realHeight = frameHeight * _rateHeight;
    ...
    values: function values() {
        var _state9 = this.state,
            ...
            imageWidth = _state9.imageWidth,
            imageHeight = _state9.imageHeight,
        ...
        if (selectionNatural && moved) {
            ...
            var realX = parseInt(thisOriginX * _rateWidth);
            var realY = parseInt(thisOriginY * _rateHeight);
